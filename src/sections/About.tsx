...
const About: React.FC = memo(() => {
  const [images, setImages] = useState<string[]>([byteImage]);

  useEffect(() => {
    // Lazy load the other images
    const loadImages = async () => {
      try {
        const [groupImage1, groupImage2] = await Promise.all([
          import("../assets/images/Image1.webp").then(module => module.default),
          import("../assets/images/Image2.webp").then(module => module.default)
        ]);
        setImages([byteImage, groupImage1, groupImage2]);
      } catch (error) {
        console.error("Failed to load images:", error);
      }
    };

    // Load images after a short delay to prioritize critical content
    ...
    <div className="flex flex-col items-center justify-center bg-gradient-to-br from-[#f8f7f3] to-[#f0ede7] text-black text-left w-full py-20 md:py-24 px-6 sm:px-6 md:px-8 relative overflow-hidden" id="About">
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
        ...
        <div className="flex justify-center items-center mt-8 lg:mt-0">
          <Carousel images={images} interval={4000} onImageLoad={handleImageLoad} />
        </div>
      </div>
    </div>
  );
